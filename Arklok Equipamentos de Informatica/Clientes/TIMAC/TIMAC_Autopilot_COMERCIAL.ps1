$host.ui.RawUI.WindowTitle = "TIMAC AUTOPILOT - PERFIL COMERCIAL"
...
if (($CHECK1 -eq 1) -or ($CHECK2 -eq 1)) {
  ...
  Start-Process -FilePath .\Timac_Autopilot_COMERCIAL.exe
  exit
}
...
while ($MICROSOFT_LOGIN_LOOP -eq $true) {
  try {
    Get-WindowsAutoPilotInfo.ps1 -online -GroupTag "TAGS-TIMAC-AGRO-BRASIL-COMMERCIAL" -Assign -ErrorAction Stop
...
Write-Host -ForegroundColor Cyan "`nTAG utilizada: TAGS-TIMAC-AGRO-BRASIL-COMMERCIAL`n"
